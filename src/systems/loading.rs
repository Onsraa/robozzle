...
pub fn load_tutorial_levels_on_enter_system(
    ...
    info!("Démarrage du chargement des tutoriels depuis: {}", tutorial_path);
    ...
    if !fs::metadata(tutorial_path).is_ok() {
        let error_msg = format!("Dossier '{}' introuvable. Créez le dossier et ajoutez vos fichiers de tutoriel 1.txt, 2.txt, etc.", tutorial_path);
        error!("{}", error_msg);
    ...
    match LevelManager::load_tutorial_levels_from_directory(tutorial_path) {
        Ok(tutorial_levels) => {
            ...
            if num_levels == 0 {
                let error_msg = format!("Aucun fichier .txt trouvé dans le dossier '{}'. Ajoutez vos fichiers de tutoriel 1.txt, 2.txt, etc.", tutorial_path);
                warn!("{}", error_msg);
                loading_state.error_message = Some(error_msg);
            ...
            info!("Transition vers l'état Tutorial avec {} niveaux", level_manager.get_levels_count());
        }
        Err(e) => {
